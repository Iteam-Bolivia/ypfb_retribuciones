...
namespace View
{
    public partial class frmCalculoManualPeriodo : Form
    {
        ...
        /// <summary>
        /// Method frmCalculoManualPeriodo
        /// </summary>
        public frmCalculoManualPeriodo()
        {
            InitializeComponent();
        }
        ...
        private void procesarPeriodo()
        {
            ...
            try
            {
                ...
                // Validar fechas, que fecha inicial sea menor o igual a fecha final
                if (year2 < year)
                {
                    Console.WriteLine("El año inicial no puede se mayor que el año final {0} {1} ", year, year2);
                }
                else
                {
                    if (year == year2)
                    {
                        if (month2 < month)
                        {
                            Console.WriteLine("El mes inicial no puede se mayor que el mes final {0} {1} {2} {3} ", year, year2, month, month2);
                        }
                        else
                        {

                            Console.WriteLine("TODO OK {0} {1} {2} {3} ", year, year2, month, month2);
                            // INICIAR PROCESO
                            // Recorrer años (i), meses (j)
                            if (year == year2)
                            {
                                Console.WriteLine("Año {0} ", year);
                                for (int j = month; j <= month2; j++)
                                {
                                    // Recorrer los meses en el mismo año
                                    Console.WriteLine("Mes {0} ", j);
                                    procesarPeriodoDetalle(ctt_id, year, j);
                            ...
                            else
                            {
                                for (int i = year; i <= year2; i++)
                                {
                                    ...
                                    if (i != year2)
                                    {
                                        ...
                                        {
                                            Console.WriteLine("Mes {0} ", j);
                                            procesarPeriodoDetalle(ctt_id, i, j);
                                        }
                                    }
                                    else
                                    {
                                        // Recorrer los meses del mismo año hasta el ultimo mes
        ...
        private void procesarPeriodoDetalle(long cttp_id, long anip_id, long mesp_id)
        {
            // Asignando parametros
            ctt_id = cttp_id;
            ani_id = anip_id;
            mes_id = mesp_id;
            //cal_id = BuscarCalId(ctt_id, ani_id, mes_id);
            ...
            //bool flag2 = false;
            //flag2 = objCalculo.listCalculoByMesAndAnio(ctt_id, ani_id, mes_id, tcl_id);
            ...
            else
            {
                ...
                try
                {
                    ...
                    #region CÓDIGO COMENTADO: Operacion que no sirve
        ...
        private long BuscarCalId(long ctt_id, long ani_id, long mes_id)
        {
            Calculo calculo = new Calculo();
            CalculoObject objCalculo = new CalculoObject();

            calculo = objCalculo.listCalculoGDT_1ByMesAndAnio(ctt_id, ani_id, mes_id, tcl_id);

            return calculo.Cal_id;
        }
            ...
            for (int k = 0; k < lstFormulas.Items.Count; k++)
            {
                ...
                switch (sVariable)
                {
                    ...
                    case "qbt":
                        ...
                        /// Valor para el campo marginal si es que existe.
                        if (CampoMargina())
                        {
                            sVariable = "qbt";
                            result = 0;
                            radbttnNoEs.Checked = false;
                            rbttnSiEs.Checked = true;
                        }
                ...
                switch (tcl_id)
                {
                    ...
                    case 1:
                        ...
                        lstContrato.ForEach(delegate(Contrato c)
                        {
                            ...
                            contrato.Ctt_costrecuacu = Convert.ToInt32(cal_costrecuacu);
        ...
        private bool CampoMargina()
        {
            ContratoMarginalObject contratoMarginalObject = new ContratoMarginalObject();
            List<ContratoMarginal> lstcontratoMarginal = contratoMarginalObject.listContratoMarginalByIdAndDate(ctt_id, mes_id,
                                                                                                       ani_id);
            long _anio_id = 0;
            long _mes_id = 0;
            long _mes_ini = 0;
            ...
            long _dia_ini = 1;
            long _dia = 1;
            bool flag_marginal = false;
            

            if(lstcontratoMarginal.Count >0)
            {
                _anio_id = lstcontratoMarginal[0].Cma_anio;
                _mes_id = lstcontratoMarginal[0].Cma_mes;
                _mes_ini = lstcontratoMarginal[0].Cma_mes_ini;
                _anio_ini = lstcontratoMarginal[0].Cma_anio_ini;
                string fecha_ini_stri = 1 + "/" + _mes_ini + "/" + _anio_ini.ToString();
                string fecha_fin_stri = 1 + "/" + _mes_id + "/" + _anio_id.ToString();
                string fecha_act_stri = 1 + "/" + mes_id + "/" + ani_id.ToString();

                DateTime fecha_ini =
                    DateTime.Parse(fecha_ini_stri);
                DateTime fecha_fin =
                    DateTime.Parse(fecha_fin_stri);
                DateTime fecha_act = DateTime.Parse(fecha_act_stri);
