...
{
    public partial class RepAnalisisRet : Form
    {
        public RepAnalisisRet()
        {
            InitializeComponent();
        }

        private void RepAnalisisRet_Load(object sender, EventArgs e)
        {
        ...
        private void CargarContratos()
        {
            try
            {
                ...
                ds = rptRedis.fillDataSetResumenEjec("fnc_analisis_retribucion", "AnalisisRetDataTable", mes_ini, anio_ini);

                ReportDataSource datasourceCon = null;

                if (ds.Tables["AnalisisRetDataTable"].Rows.Count > 0)
                {
                    datasourceCon = new ReportDataSource("DataSet1", ds.Tables["AnalisisRetDataTable"]);
