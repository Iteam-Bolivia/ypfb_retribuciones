...
{
    public partial class RepResumenEjec : Form
    {
        public RepResumenEjec()
        {
            InitializeComponent();
        }

        private void RepResumenEjec_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void CargarContratos()
        {
            try
            {
                ...
                ds = rptRedis.fillDataSetResumenEjec("fnc_resumen_ejecutivo", "ResumenEjecDataTable", mes_ini, anio_ini);

                ReportDataSource datasourceCon = null;

                if (ds.Tables["ResumenEjecDataTable"].Rows.Count > 0)
                {
                    datasourceCon = new ReportDataSource("DataSet1", ds.Tables["ResumenEjecDataTable"]);
