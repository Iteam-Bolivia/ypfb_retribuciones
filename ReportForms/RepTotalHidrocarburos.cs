...
{
    public partial class RepTotalHidrocarburos : Form
    {
        public RepTotalHidrocarburos()
        {
            InitializeComponent();
        }

        private void RepTotalHidrocarburos_Load(object sender, EventArgs e)
        {
        ...
        private void CargarContratos()
        {
            try
            {
                ...
                ds = rptRedis.fillDataSetResumenEjec("fnc_totalhidrocarburos", "DistribRetOpeDataTable", mes_ini, anio_ini);

                ReportDataSource datasourceCon = null;

                if (ds.Tables["DistribRetOpeDataTable"].Rows.Count > 0)
                {
                    datasourceCon = new ReportDataSource("DataSet1", ds.Tables["DistribRetOpeDataTable"]);
                    reportViewer1.LocalReport.DataSources.Clear();

                    //Muestro los datos en el DataGrid
                    //this.dataGridView1.DataSource = ds.Tables["DistribRetOpeDataTable"];
